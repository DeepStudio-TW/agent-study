...
"""
        ...
        if next_message is not None:
            ...
            # 使用新的格式化方法
            formatted_result = self._format_agent_results(result)
        else:
            result = None
            formatted_result = "No results available"
        
        # cache the result
        self.agent_extraction[tinyperson.name] = result

        return formatted_result
    def _format_agent_results(self, results):
        """Format the agent extraction results in a readable way."""
        if not isinstance(results, dict):
            return str(results)

        output = []
        output.append("\n" + "="*60)
        output.append("   AI聊天機器人創新功能討論總結")
        output.append("="*60 + "\n")
        ...
        if "summary" in results:
            ...
            if "main_points" in summary:
                output.append("📌 主要討論要點")
                output.append("-"*30)
                for point in summary["main_points"]:
                    output.append(f"  • {point}")
                output.append("")

            if "innovations" in summary:
                output.append("💡 創新功能提案")
                output.append("-"*30)
                for innovation in summary["innovations"]:
                    output.append(f"  • {innovation}")
                output.append("")

            if "technical_considerations" in summary:
                output.append("⚙️ 技術實現考量")
                output.append("-"*30)
                for consideration in summary["technical_considerations"]:
                    output.append(f"  • {consideration}")
                output.append("")

            if "challenges" in summary:
                output.append("⚠️ 潛在挑戰")
                output.append("-"*30)
                for challenge in summary["challenges"]:
                    output.append(f"  • {challenge}\n")    # 每個挑戰後添加空行，增加可讀性

            if "team_contributions" in summary:
                output.append("\n👥 團隊成員貢獻")      # 在團隊貢獻前添加額外的空行
                output.append("-"*30)
                for member, contribution in summary["team_contributions"].items():
                    output.append(f"  🔹 {member}")
                    output.append(f"    {contribution}")
                    output.append("")                    # 每個成員之間添加空行
