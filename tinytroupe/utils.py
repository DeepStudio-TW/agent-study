...
# tinytroupe/utils.py
from tinytroupe.config import config  # 導入配置實例
...
# Prompt engineering
################################################################################
def add_rai_template_variables_if_enabled(template_variables):
    """Add RAI template variables if enabled in config."""
    
    # 正確使用 config 實例的 getboolean 方法
    rai_harmful_content_prevention = config.getboolean(
        "Simulation", 
        "RAI_HARMFUL_CONTENT_PREVENTION"
    )
    
    rai_copyright_infringement_prevention = config.getboolean(
        "Simulation", 
    ...
    if rai_harmful_content_prevention:
        ...
        Important: Never generate harmful, dangerous, or illegal content.
        Always maintain ethical and safe behavior.
        """

    if rai_copyright_infringement_prevention:
        template_variables["rai_copyright_infringement_prevention"] = """
        Important: Respect intellectual property rights.
        Do not copy or infringe upon copyrighted material.
        """
